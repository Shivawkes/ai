...
const titles = ['san', 'kun', 'kimi', 'chan', 'sama', 'sensei'];
...
export default class extends Module {
	...
	private transferBegin(msg: Message): boolean  {
		if (!msg.text) return false;
		if (!msg.includes(['take over', 'handover', 'moving', 'moving house'])) return false;
	...
	private setName(msg: Message): boolean  {
		...
		const name = msg.extractedText.match(/\s*(.+?)Call me/)![1];
